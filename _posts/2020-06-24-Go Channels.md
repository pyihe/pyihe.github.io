...
**通道操作情形C：** <u>当一个协程尝试关闭一个非空且未关闭的通道</u>，一旦该程获得了该通道的锁，以下两个步骤都会按照下面的顺序被执行：
...
**通道操作情形D：** <u>在非空通道关闭后，该通道上的通道接收操作（译者注：从该通道接收数据的操作）将永远不会阻塞</u>。该通道数据缓冲区中的数据仍然可以被接收，附随的第二个可选的布尔返回值仍然为`true`。一旦数据缓冲区中的所有数据被取出和接收，无数个通道元素类型的零值将会被任何该通道接收操作对应的通道所接收。正如上面提到的，通道接收操作可选的第二个返回结果是一个无类型的布尔值，它暗示着接收操作的第一个返回值是否是在通道关闭之前发送的。如果第二个返回结果为`false`，此时接收操作的第一个返回值一定是通道元素类型的零值。
