...
1. RSA的应用
    ...
    * 数字签名
    ...
    关于RSA加密原理的具体实现有很多书籍有很细致的讲解，参考《深入浅出密码学——常用加密技术原理与应用》。
...
但在信道中传输明文是危险的，所以通常做法是，对明文进行加密后在传输，通常使用Hash对明文进行加密，以确保数据完整性

接收方收到签名数据后，首先用公钥对密文进行解密，得到明文数据，然后通过双方商定的明文Hash方式加密解密出来的明文，在将结果与收到的Hash明文对比，如果一致则证明数据有效，否则数据无效。
